...
def main():
    ...
    required=['R3 DEVELOPMENT TEST LOOP','/api/dev/core-dump','rollamacoaster-r3','/api/tags','/api/chat','REVISION 3','R3.0-dev.5','updateKeyboardCamera(dt)','WASD camera translation','cam.position.distanceTo(ctl.target)','frameStep(\"camera\"','frame-loop fault containment']
    missing=[x for x in required if x not in text]
    tests.append({'cmd':'required markers','ok':not missing,'stdout':'','stderr':('missing: '+', '.join(missing)) if missing else ''})
    forbidden=['cam.distanceTo(ctl.target)']
    found_forbidden=[x for x in forbidden if x in text]
    tests.append({'cmd':'camera API regression guard','ok':not found_forbidden,'stdout':'','stderr':('forbidden: '+', '.join(found_forbidden)) if found_forbidden else ''})
