...
## R3.0-dev.19 focused playtest

1. Boot and confirm the main command rail is exactly one icon tall. Click `⌄⌄` and confirm labels/full menu expand; click `⌃⌃` to collapse.
2. Press Escape → New. Confirm no browser dialog appears. The in-game New Park panel must show Cancel and Start New Park buttons.
3. Cancel once and verify the current park remains untouched. Open New again and start a fresh park.
4. Open Frontier Operations and verify the live entrance camera and operating dashboard remain intact. Open Entity Registry.
5. Confirm Entity Registry reports `entityTemplates` and `entityInstances` architecture and displays UUID/template relationships.
6. Place several paths, scenery pieces and a ride; hire staff; open the park and admit guests. Re-open Entity Registry and confirm instance count grows.
7. Save and reload. Inspect several entities/guests and confirm UUIDs survive the save/load round trip.
8. Hover a ride, shop/path, guest, entrance and fence. Confirm the hover card shows useful live status, a logo, lore and DB identity rather than only a name/coordinate.
9. Export a review packet and verify DB diagnostics include nonzero `entityTemplates` and `entityInstances` counts.

## R3.0-dev.23 focused playtest

1. Boot and confirm the Command Deck still appears front-and-center over a staged live world.
2. Click HUD cash and confirm Frontier Ledger opens. Click guest count and confirm Guest Database opens, including cap slider/exact value, FrontierOS broadcast and PA announcement controls.
3. Click ⭐ rating and 📣 popularity. Confirm Park Metrics + Reviews opens, graphs accumulate samples, guest review rows are clickable, and Backspace/mouse-Back closes the manager.
4. Confirm the clock shows a current weather logo/temperature. Click it and inspect the 15-hour forecast.
5. Let the park run through several weather blocks and verify world particles visibly switch among rain, wind-blown leaves, dust/mist conditions without frame faults.
6. Hold Space and confirm the camera + orbit target lift together. Try Space+WASD and Shift+Space. Focus a text/chat input and confirm Space types normally instead of lifting the camera.
7. Admit enough guests to compare adult/teen/child scales, masculine/feminine visual presentations, costume variations and carried props.
8. Hire Handyman, Security, Entertainer and Construction staff. Confirm each has a distinct uniform/prop and opens as a draggable entity window.
9. Wait for pets to appear, right-click one, test leash/unleash, and verify its UUID-backed entity inspector and autonomous movement.
10. Watch guests for grass crossing, group-circle/social movement, weather watching, pet/wildlife observation and geocache seeking.
11. Keep two social guests near each other long enough to form a relationship. Open both Guest Minds and verify relationship/contact/address-book state and periodic knowledge transfer.
12. Open FrontierOS from the main toolbar and from a Guest Mind. Exercise Music, Texts, Digital Park Map and Personal Notes. Send an admin broadcast and a PA announcement.
13. Export the review packet. Confirm `database.entityInstances` grows with guests/pets, `context.weather` is populated, `context.metrics.samples` is nonzero, `context.camera.input.keysDown` can record `Space`, and errors/frameFaults remain empty.

## R3.0-dev.24 acceptance
See `DEV24_REVIEW_ACCEPTANCE.md` for the review-to-implementation matrix and next focused playtest.
