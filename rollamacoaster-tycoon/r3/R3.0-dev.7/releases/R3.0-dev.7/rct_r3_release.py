...
def main():
    ...
    required=['R3 DEVELOPMENT TEST LOOP','/api/dev/core-dump','rollamacoaster-r3','/api/tags','/api/chat','REVISION 3','R3.0-dev.7','updateKeyboardCamera(dt)','cam.position.distanceTo(ctl.target)','frameStep(\"camera\"','frame-loop fault containment','R3_GRID_FENCE_RELEASE_NOTE','GRID_STEP=2','makeConstructionGrid()','makeParkBoundaryFence()','pathCellInside(x,z)','exact 2m grid cell','R3_SIGN_PATH_RELEASE_NOTE',"mountedLabelPlane('FOREST FRONTIERS'",'const w=k===\"queue\"?1.18:GRID_STEP,depth=GRID_STEP']
