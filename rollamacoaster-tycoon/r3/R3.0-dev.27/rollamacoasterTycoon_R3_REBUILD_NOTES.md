...
## R3.0-dev.11 — authoritative interaction router
...
## R3.0-dev.12 — menu-first Frontier Operations
- Removed the legacy startup title card and made the Escape Command Deck the single main-menu surface.
- Reworked Park Manager into branded module navigation with a live entrance camera, Gateworks, Entrance Identity, Admission/Capacity, and Land Office.
- Kept FrontierNet outside Park Manager as an independent system surface.
- Added guest-cap slider plus exact-number control in Park Manager and the main HUD.

## R3.0-dev.14 — settings/systems hierarchy + admission economics
- Moved Construction Grid and Render Quality into a dedicated Settings manager placed before Systems in the main toolbar.
- Replaced the former Systems dropdown with a real Frontier Systems parent manager. Child managers remain layered above it and close with Backspace or the mouse Back button.
- Removed Park Display, Park Pulse, and Alexandria shortcuts from Frontier Operations. Park Manager now stays focused on park admission, identity, Gateworks, capacity, and land.
- Moved the live entrance camera to the top of Frontier Operations.
- Removed browser `prompt()` workflows. Exact guest cap, entrance fee, save-as name, and Flock collective prompt are edited directly in the UI.
- Park admission is authoritative from $0 through a hard maximum of $200, including load/migration normalization.
- Candidate guest wallets use a triangular $20–$220 distribution with an exact $120 mean. Guests who cannot cover admission decline entry instead of receiving a negative wallet.
- Core diagnostics expose the active economy-rule constants alongside UI/menu state.

## R3.0-dev.17 — Guest Mind / Arrival / Visual Evidence pass

- Guests now spawn at the wilderness map-edge terminus and physically walk the public approach before admission is evaluated at the entrance.
- Each guest receives a persistent individual profile: archetype, origin, ride-intensity tolerance, curiosity, patience, spending preference, social tendency, food adventurousness, walking pace, interests, needs, inventory and lore.
- Destination choice is weighted per person using need urgency, ride-intensity fit, queue pressure, patience, affordability, novelty, interests, curiosity, prior visits and distance instead of a shared global priority tree.
- Every active non-resting guest maintains a ten-point GPS forecast. Resting guests intentionally suspend the route.
- Guest meshes are raycast-addressable. Right-click and Interact-click route directly to the individual guest mind window.
- Guest windows expose live activity, mood, needs, inventory, persistent brain counts, ten-point GPS route, live local/Ollama inner thought and direct first-person Ollama chat.
- Background Ollama thought generation rotates conservatively across guests rather than firing one request per guest per frame.
- Entrance typography now previews the selected font stack using the actual sign text.
- Gateworks architecture families carry theme colors; masonry finishes are visual swatches with material lore.
- Full viewport captures are cached in sessionStorage and IndexedDB and reloaded into the review packet before export, eliminating the dev.15 `visualCapture: null` failure mode.
- Missing manager symbols discovered by semantic validation were implemented: Attraction Works, Frontier Crew, Frontier Ledger, Alexandria search, Park Archives, list/delete save helpers and ledger-to-entity routing.
- The release gate now includes a TypeScript semantic unresolved-identifier scan in addition to JavaScript syntax and deterministic-build checks.
...
## R3.0-dev.18 — Credential projection + collapsed ledger

The dev.17 review packet proved admission accounting itself was correct, but Guest Mind could display stale credentials when it remained open across gate admission. Dev.18 introduces one idempotent credential normalizer used by new guests, admission, save migration, and live entity refresh. Admission tickets therefore exist in authoritative state and become visible immediately without reopening the guest window.

Frontier Ledger now treats history as drill-down data. The latest transaction remains visible while the full transaction history is inside a closed-by-default `details` surface, preventing hundreds of ledger rows from dominating the management window.

## R3.0-dev.19 — Entity repository + Command Deck New UI + compact command rail

- IndexedDB schema upgraded to v4 with `entityTemplates` and `entityInstances` stores.
- Canonical catalog, actor and infrastructure templates are stored once. Live world objects, guests, staff, park root, entrance, boundary, wilderness approach and property parcels receive stable UUIDs and persist as template references plus diffs.
- Added an Entity Registry module to Frontier Operations so template/instance counts and UUID/template relationships are visible in-game.
- Save, load, new-park, boot and manager refresh flows synchronize the entity repository; legacy state receives UUIDs during normalization.
- Command Deck New no longer invokes a browser confirmation dialog. It opens an in-game New Park surface with Cancel and Start New Park buttons.
- The main command rail defaults to one icon tall. A `⌄⌄` button expands labels/full menu; `⌃⌃` collapses it again.
- Hover cards now expose active state, value/price, queue/rider counts, guest mood/wallet/credentials, template identity, UUID, richer logos and contextual lore.
- Frontier Operations preserves the approved visual hierarchy: live entrance camera first, park charter/status, Open/Close Park, Entrance Identity, Gateworks, Admission/Capacity and Land Office.

## R3.0-dev.23 — review-file HUD, atmosphere, actors, FrontierOS, camera lift

Built directly from the R3.0-dev.22 review packet.

- HUD drill-down contract: cash opens Frontier Ledger; guest count opens Guest Database; rating/popularity open Park Metrics + Reviews; time carries current weather and opens forecast.
- Guest Database now includes live guest-cap controls, FrontierOS broadcast-to-all, and PA announcement controls.
- Added Park Metrics + Reviews with sampled rating/popularity history, inline SVG trend graphs, guest thoughts and clickable reviews.
- Added deterministic simulated weather with current condition + 15-hour forecast. Conditions drive visible rain, wind-blown leaves, dust motes and mist particles in the 3D world.
- Every new child manager added in this revision is a normal draggable `.panel`, inheriting Backspace and mouse-Back close behavior from the authoritative window manager.
- Guest appearance profiles now include adult/teen/child scale, masculine/feminine presentation, costume family and occasional carried props. These are visual presentation fields, separate from guest personality/brain data.
- Staff now include themed role uniforms and carried props; construction worker is a first-class staff role alongside handyman, security and entertainer.
- Added persistent pet entities with UUID/template storage, autonomous wandering, leash/unleash behavior, world meshes, hover status and entity inspector.
- Guest free-time behavior can cross grass, join social circles, watch pets/wildlife, react to weather, or search for geocaches instead of targeting the only nearby bench.
- Guest social encounters can form relationships, exchange FrontierOS contacts/address-book entries and transfer persistent knowledge. The knowledge broker is throttled so it remains periodic.
- FrontierOS now exists both as a main admin manager and per-guest phone surface. Apps include Music, Texts, Digital Park Map and Personal Notes.
- Spacebar is part of the camera contract: hold Space to lift both camera and OrbitControls target; Space can combine with WASD and Shift acceleration and is disabled while typing.
- Core diagnostics now include pet count, current/forecast weather and metrics-history state.

## R3.0-dev.27 — Alexandria becomes native gameplay infrastructure
- Automatic LoA health and reconnect monitoring that never blocks simulation.
- Bounded automatic guest research based on curiosity, interests, activity and destinations.
- Native guestBrain adoption, first-person conversation context, capped decision bias and save persistence.
- Systems > Alexandria exposes live status and live Library search without DevTools.
- Guest inspectors expose retained knowledge and service state; manual research remains an in-game option.
