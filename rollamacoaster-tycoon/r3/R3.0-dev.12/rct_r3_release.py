...
def main():
    ...
    required=['R3 DEVELOPMENT TEST LOOP','/api/dev/core-dump','rollamacoaster-r3','/api/tags','/api/chat','REVISION 3','R3.0-dev.12','updateKeyboardCamera(dt)','cam.position.distanceTo(ctl.target)','frameStep("camera"','frame-loop fault containment','R3_GRID_FENCE_RELEASE_NOTE','GRID_STEP=2','makeConstructionGrid()','makeParkBoundaryFence()','pathCellInside(x,z)','exact 2m grid cell','R3_SIGN_PATH_RELEASE_NOTE','R3_ENTRANCE_PROPERTY_RELEASE_NOTE','makeWildernessApproachPath()','PROPERTY_PARCELS','parkSignFont','boundaryPartFromHits(hits)','data-buy-parcel',"rightClick:'fence=property, entrance=park-manager'",'const w=k==="queue"?1.18:GRID_STEP,depth=GRID_STEP','R3_CINEMATIC_UI_RELEASE_NOTE','R3_GATEWORKS_RELEASE_NOTE','R3_TRANSACTION_FEEDBACK_RELEASE_NOTE','R3_COMMAND_DECK_RELEASE_NOTE','R3_DIAGNOSTIC_CAPTURE_RELEASE_NOTE','R3_INTERACTION_ROUTER_RELEASE_NOTE','R3_PARK_MENU_RELEASE_NOTE','routeBoundaryInteraction(part','interactionPolicy=\'park-manager-only\'','lastInteractionRoute','activeParkBounds()','activeParkGate()','ensureGateConnectorPaths()','systemTray','positionSystemsTray()','toggleSystemsTray','pauseMenu','openPauseMenu','renderPauseMenu','menuContinue','menuNew','startNewPark(confirmExisting=false)','menuSave','menuLoad','menuCheats','menuExit','data-menu-delete','Command Deck active','preserveDrawingBuffer:true','forced-render-dataurl','networkPanel','pulsePanel','simRateSlider','guestCapEditor','guestCapSlider','guestCapExact','parkMenuGrid','parkMenuTile','parkGuestCapSlider','parkGuestCapExact','entranceLive','GATE_STYLES','MASONRY_STYLES','insufficientFunds','showTransactionFloat','SYSTEM_BRANDS','FRONTIERNET','GATEWORKS','SURVEYOR']
    missing=[x for x in required if x not in text]
    tests.append({'cmd':'required markers','ok':not missing,'stdout':'','stderr':('missing: '+', '.join(missing)) if missing else ''})
    forbidden=['cam.distanceTo(ctl.target)','new THREE.GridHelper(160,80',"labelSprite('FOREST FRONTIERS'",'const w=k==="queue"?1.18:1.82','#systemTray{position:fixed;z-index:99',"if(e.key==='Escape'||e.key==='Backspace')",'if(toolMode==="interact"){const e=rootEntityFromHit(hits[0]);if(e)openEntity(e.id)}']
    forbidden.extend(['<div id="title"><div class="cardTitle">','openNetworkFromPark','guestLimitNumber','guestCapInput','guestCapApply'])
