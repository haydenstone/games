...
const CODEX_PATH=process.env.CODEX_PATH||'/codex/codex.json';
fs.mkdirSync(DATA,{recursive:true});
if(!fs.existsSync(CODEX_PATH)){ console.error(`R5 startup blocked: Codex not found at ${CODEX_PATH}. Mount your host codex.json read-only to /codex/codex.json.`); process.exit(2); }
let codex; try { codex=JSON.parse(fs.readFileSync(CODEX_PATH,'utf8')); } catch(err) { console.error(`R5 startup blocked: Codex at ${CODEX_PATH} is not valid JSON: ${err.message}`); process.exit(2); }
...
const initial={version:'R5.0-dev.4',tick:0,simulationRunning:true,codexAvailable:true,entity:{timeline:0,happiness:55,wisdom:0,lastPerception:null},events:[]};
...
server.listen(PORT,HOST,()=>console.log(`R5 dev.4 listening on http://${HOST}:${PORT}`));
